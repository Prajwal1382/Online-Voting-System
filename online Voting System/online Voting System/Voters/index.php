...
?>
    <div class="row my=3">
        <div class="col-12">
        <style> 
#bg-green {background-color: #67C232;}
#text-white {color: #fff;}
</style>
            <h3> Voters Panel </h3>
            
            <?php
                $fetchingActiveElections = mysqli_query($db, "SELECT * FROM elections WHERE status = 'Active'")
                ...
                <thead>
                    <tr>
                        <th colspan="4" id="bg-green" my-3><h5 id="text-white"> ELECTION TOPIC: <?php echo
                ...
                <tbody>
                    <?php
                        ...
                        if(isset($candidateData['id'])){
                            ...
                            $candidate_id = null;
                        }if(isset($candidateData['candidate_photo'])){
                            $candidate_photo = $candidateData['candidate_photo'];
                        }else{
                            $candidate_photo = null;
                        }
                               
                            //Fetching Candidate Voters
                            $fetchingVotes = mysqli_query($db, "SELECT * FROM votings WHERE candidate_id =
                            '".$candidate_id."'") or die(mysqli_error($db));
                    ...
                    ?>
                             ...
                             <tr>
                                ...
                                <td>
                                <?php
                                     $checkIfVoteCasted = mysqli_query($db, "SELECT * FROM votings WHERE 
                                     voters_id = '".$_SESSION['user_id']."' AND election_id = '".
                                     $election_id."'") or die(mysqli_error($db));
                                     $isVoteCasted = mysqli_num_rows($checkIfVoteCasted);
                                     
                                     if($isVoteCasted > 0)
                                     {
                                        $isVoteCastedData = mysqli_fetch_assoc($checkIfVoteCasted);
                                        $isVoteCastedToCandidate = $voteCastedData['candidate_id'];

                                        if($isVoteCastedToCandidate == $candidate_id)
                                        {
                                         ?>
                                            <img src="../assest/images/vote.png" width="100px;">
                                         <?php 
                                        }
