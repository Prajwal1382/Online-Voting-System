...
?>
    <div class="alert alert-danger my-3" role="alert">
       Invalide image type (Only .jpg, .png files are allowed) .
     </div>
<?php
...
#candidate_photo {
    ...
    <div class="col-4">
        ...
        <form method="POST" enctype="multipart/form-data">
                ...
                <select class="form-control" name="election_id" required>
                    ...
                    <?php
                       ...
                       if($isAnyElectionAdded > 0)
                       {
                        ...
                        //Now checking how many candidates are added in this elections
                        $fetchingCandidate = mysqli_query($db, "SELECT * FROM candidate_details WHERE
            ...
            <div class="form-group">
                <input type="text" name="candidate_name" placeholder="No of Candidates" class="form-control" required />
            </div>
            <div class="form-group">
  ...
  <tbody>
    ...
    if($isAnyCandidateAdded > 0)
    {
        ...
        while($row = mysqli_fetch_assoc($fetchingData))
        {
            ...
            $election_id."'") or die(mysqli_error($db));
        ...
        ?>
                        <tr>
                            <td><?php echo $sno++; ?></td>
                            <td><img src="<?php echo $candidate_photo; ?>" id="candidate_photo" />  </td>
                            <td><?php echo $row['candidate_name']; ?></td>
                            <td><?php echo $row['candidate_details']; ?></td>
                            <td><?php echo $election_name; ?></td>
                            <td>
                                <a href="#" class="btn-sm btn-warning"> Edit </a>
                                <button class="btn-sm btn-danger" onclick="DeleteData(<?php echo
                                $candidate_details; ?>)"> Delete </button>
                            </td>
...
<?php
      if(isset($_POST['addCandidateBtn']))
      {
        ...
        // Photograpgh Logic starts
        $targetted_folder = "../assest/Images/candidate_photos/";
        $candidate_photo = $targetted_folder . rand(111111111, 99999999999) ." ". rand(111111111, 99999999999) . 
        ...
        $image_size = $_FILES['candidate_photo']['size'];
         ...
         { 
            if(in_array($candidate_photo_type, $allowed_types))
            {
                 ...
                 {
                     //inserting into db
                        mysqli_query($db, "INSERT INTO candidate_details(election_id, candidate_name, candidate_details, 
                        candidate_photo, inserted_by, inserted_on) VALUES('".$election_id."','".$candidate_name."','".
